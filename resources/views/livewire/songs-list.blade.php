<div>
    ...
    <div class="row bg-light rounded-3 p-3 shadow-sm">
        ...
        <div class="col-md-4">
            <form class="d-flex">
                <input wire:model.live.debounce.300ms="query" class="form-control form-control-sm shadow-sm"
                    type="search" placeholder="Cari Pujian..." aria-label="Search">
            </form>
        </div>
    </div>
    {{-- end header --}}
                        ...
                        <div class="card h-100 border-0 shadow-sm transition-card">
                            ...
                            <div class="card-body">
                                ...
                                <div class="author mb-1 text-muted" style="font-size: 0.6rem">
                                    By <a href="{{ route('authors.songs', $song->author->username) }}"
                                        class="text-muted">{{ $song->author->name }}</a>
                                    {{ $song->author->articles_count }}
                                    in
                                    <a href="{{ route('categories-songs.show', $song->categorysong->slug) }}"
                                        class="text-muted">{{ $song->categorysong->name }}</a>
